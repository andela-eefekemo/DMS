...
class AccessActions {
  ...
  */
  static signUpUser(userDetails) {
    return dispatch => axios.post('/api/v1/users', userDetails)
      .then((response) => {
  ...
  */
  static signInUser(userDetails) {
    return dispatch => axios.post('/api/v1/users/login', userDetails)
      .then((response) => {
        if (
          response.status === 200) {
          setAuthorizationToken(response.data.token);
  ...
  */
  static signOutUser() {
    return dispatch => axios.post('/api/v1/users/logout')
      .then((response) => {
